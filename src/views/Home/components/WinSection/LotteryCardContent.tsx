...
const LotteryCardContent = () => {
  ...
  const cakePrizesText = t('%cakePrizeInUsd% in ORB prizes this round', { cakePrizeInUsd: cakePriceBusd.toString() })
  const [pretext, prizesThisRound] = cakePrizesText.split(cakePriceBusd.toString())
  const amountCollectedInCake = currentLottery ? parseFloat(currentLottery.amountCollectedInOrb) : null
    ...
    <>
      <Flex flexDirection="column" mt="48px">
        ...
        <Text color="white" mb="40px">
          {t('Buy tickets with ORB, win ORB if your numbers match')}
        </Text>
      </Flex>
