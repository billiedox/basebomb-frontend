...
const PreviousRoundCardFooter: React.FC<{ lotteryNodeData: LotteryRound; lotteryId: string }> = ({
  ...
  if (lotteryNodeData) {
    const { amountCollectedInOrb } = lotteryNodeData
    prizeInBusd = amountCollectedInOrb.times(cakePriceBusd)
  }
      ...
      <>
        ...
        {prizeInBusd.isNaN() ? (
          ...
          <Balance
            ...
            color="textSubtle"
            unit=" ORB"
            value={getBalanceNumber(lotteryNodeData?.amountCollectedInOrb)}
            decimals={0}
          />
        )}
      </>
