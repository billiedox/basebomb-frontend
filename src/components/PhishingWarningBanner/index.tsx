...
const Container = styled(Flex)`
  ...
  padding: 12px;
  align-items: center;
  background: linear-gradient(0deg, rgba(54, 23, 54, 0.4), rgba(54, 23, 54, 0.4)),
    linear-gradient(180deg, #762a95 0%, #361736 100%);
  ${({ theme }) => theme.mediaQueries.md} {
    padding: 0px;
    background: linear-gradient(180deg, #762a95 0%, #361736 100%);
  }
`
...
const PhishingWarningBanner: React.FC = () => {
  ...
  const warningText = t("please make sure you're visiting https://opbomb.finance - check the URL carefully.")
  const warningTextAsParts = warningText.split(/(https:\/\/opbomb.finance)/g)
    ...
    <>
      ...
      {warningTextAsParts.map((text, i) => (
        <Text
          ...
          small
          as="span"
          bold={text === 'https://opbomb.finance'}
          color={text === 'https://opbomb.finance' ? '#FFFFFF' : '#BDC2C4'}
        >
          {text}
        </Text>
      ))}
