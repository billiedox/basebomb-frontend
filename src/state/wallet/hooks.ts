...
export function useETHBalance(account?: string) {
  const { library } = useWeb3React()
  const [ethBalance, setETHBalance] = useState<CurrencyAmount>()
  useEffect(() => {
    (async () => {
      const signer = library.getSigner(account)
      const balance = await signer.getBalance()
      setETHBalance(CurrencyAmount.ether(JSBI.BigInt(balance.toString())))
    })()
  }, [account, library])
  return ethBalance
}
